    ...
    var handleRecords = function () {
        ...
        grid.init({
            ...
            dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options 
                ...
                "ajax": {
                    "url": "/Admin/Slide/LoadTable", // ajax source
                },
        ...
        $('#frmSaveData').validate({
            ...
            messages: {
                txtTitle: {
                    ...
                    maxlength: " Tối đa chỉ 30 ký tự"
                },
                txtImage: {
                    required: " Bạn phải chọn hình"
                }
            }
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status == true) {
                    ...
                    $('#txtPublishDate').val(data.publishDate);
                    $("img#output").attr('src', "../" + data.image);
                    $("#summernote_1").summernote('code', data.description);

                    data.status == 1 ? $('#txtStatus').bootstrapSwitch('state', true) : $('#txtStatus').bootstrapSwitch('state', false);
